...
一个技术上完美但商业上无利可图的智能体，在生产环境中是一个失败品。本条目引入一个务实的、至关重要的原则：智能体的架构必须从一开始就为经济可行性进行设计，成本应被视为与性能和准确性同等重要的一级架构约束。

#### 原理与论证

智能体开发的另一个“残酷的数学现实”是，天真的会话式智能体存在二次方级扩展的令牌成本。因为在许多实现中，为了维持上下文，每一次交互都会将完整的历史记录重新提交给模型。这意味着对话越长，单次调用的成本就越高，一个包含100轮对话的交互可能仅令牌成本就高达数十美元，这对于大多数应用来说是不可持续的。
此外，智能体的总拥有成本远不止API调用费用，还包括昂贵的GPU计算、存储和基础设施维护费用。正因如此，最新的智能体评估基准已经开始将成本作为与任务成功率并列的核心评估指标。
因此，架构师必须主动采用能够控制成本的模式：
- 模型路由/级联（Model Routing/Cascading）：并非所有任务都需要最强大、最昂贵的模型。一个高效的模式是使用一个更小、更便宜、更快的模型（如GPT-4o-mini, Claude 3.5 Sonnet）作为“路由器”或处理简单子任务。只有当任务被识别为高度复杂或需要深度推理时，才将其“升级”到一个更强大的模型（如GPT-4.5, Claude 3.5 Opus）来处理 7。这本质上是一种以成本优化为目标的多智能体设计。
- 主动的上下文管理（Aggressive Context Management）：与其被动地填充上下文窗口直至溢出，不如主动管理它。有效的策略包括：定期使用LLM对早期对话进行摘要，用一个简短的总结来代替冗长的原文；或者实现一个滑动窗口，只保留最近的N轮对话。这些策略打破了成本的二次方级增长曲线。
- 优先选择无状态设计（Stateless-First Design）：架构师必须审慎地质问：这个任务真的需要一个有状态的、持续的对话吗？对于许多用例，一个无状态的、工具般的智能体（接收输入、执行任务、返回输出）远比一个有状态的会话智能体更可靠、更便宜。在不需要长期记忆的情况下，应优先选择无状态设计，以避免不必要的复杂性和成本。

#### 可操作案例

Claude Code 的智能核心由 Anthropic 最先进的模型家族驱动，特别是为实现最高性能而采用的 Claude Opus 4.1。整个系统架构建立在 Opus、Sonnet 和 Haiku 这一模型层级之上。这种设计允许系统根据任务的复杂度和成本效益进行动态调整，例如，使用强大的 Opus 模型进行复杂的顶层规划，然后切换到更具成本效益的 Sonnet 模型来执行具体的实现任务 ，大文件的读取用Haiku。
